...
The top level being called [`alarm_clock_top`](alarm_clock/alarm_clock.srcs/sources_1/new/alarm_clock_top.vhd) <br>
...
Note: once the countdown starts, the only responding buttons are the center (stop) and left button (reset).

## List of used hardware components on the board
| Component | Function |
| :--- | :----- |
...
## Modules
Everything is wrapped into top level [`alarm_clock_top.vhd`](alarm_clock/alarm_clock.srcs/sources_1/new/alarm_clock_top.vhd)

Some already existing modules from the [VHDL examples](https://github.com/tomas-fryza/vhdl-examples/tree/master) are used within this project. Clicking the name takes you into this projects code, clicking anything in the simple brackets redirects you to the corresponding source. 
- [```bin2seg.vhd```](alarm_clock/alarm_clock.srcs/sources_1/imports/imports/new/bin2seg.vhd) (modified, the default number is set to 0 (seg <= 0000001), otherwise works completely same) ([docs](https://github.com/tomas-fryza/vhdl-examples/tree/master/lab3-segment), [code](https://github.com/tomas-fryza/vhdl-examples/blob/master/examples/_solutions/lab6-debounce/bin2seg.vhd))
- [```clk_en.vhd```](alarm_clock/alarm_clock.srcs/sources_1/imports/imports/new/clk_en.vhd) (unchanged) ([docs](https://github.com/tomas-fryza/vhdl-examples/tree/master/lab4-counter), [code](https://github.com/tomas-fryza/vhdl-examples/blob/master/examples/_solutions/lab6-debounce/clk_en.vhd))
- [```counter.vhd```](alarm_clock/alarm_clock.srcs/sources_1/imports/imports/Documents/counter/counter.srcs/sources_1/new/counter.vhd) (unchanged) ([docs](https://github.com/tomas-fryza/vhdl-examples/tree/master/lab4-counter), [code](https://github.com/tomas-fryza/vhdl-examples/blob/master/examples/_solutions/lab6-debounce/counter.vhd))
- [```display_driver.vhd```](alarm_clock/alarm_clock.srcs/sources_1/imports/imports/new/display_driver.vhd) (modified, allowing 6 segment displays instead of only 2) ([docs](https://github.com/tomas-fryza/vhdl-examples/tree/master/lab5-display), [code](https://github.com/tomas-fryza/vhdl-examples/blob/master/examples/_solutions/lab6-debounce/display_driver.vhd))
- [```debounce.vhd```](alarm_clock/alarm_clock.srcs/sources_1/imports/imports/Documents/debounce/debounce.srcs/sources_1/new/debounce.vhd) (unchanged) ([docs](https://github.com/tomas-fryza/vhdl-examples/tree/master/lab6-debounce), [code](https://github.com/tomas-fryza/vhdl-examples/blob/master/examples/_solutions/lab6-debounce/debounce.vhd))
...
Project [constraints file](https://github.com/Patik05/BPC-DE1-Project-2026/blob/main/alarm_clock/alarm_clock.srcs/constrs_1/imports/newly/nexys.xdc)

(Newly made/functionaly heavily modified) modules are later documented in this markdown file.

## Custom module [`alarm_clock.vhd`](alarm_clock/alarm_clock.srcs/sources_1/new/alarm_clock.vhd)

### Background
An alarm clock is used to be an adjustable timer that signals to an user that time has run out. It also requires to have a possible way for an user to input desired time values and to somehow signal the countdown to the user.

### I/O ports of the `alarm_clock.vhd` module:
...
The module essentially operates with three states: `Setup mode` (paused, being editable) and a `Countdown mode` (when the time is counting down) and `Alarm ringing mode` (time has run out). These modes are decided by `counting_down` and `is_zero` flags.

| Mode | `counting_down` | `is_zero` | What it does |
| :---:| :---:| :---: | :--- |
|`Setup mode`| 0 | 0 | User interacts with time incrementation/decrementation (`sig_inc`, `sig_dec` respectively). The math uses a wrap-around logic (meaning that decrementing 00 seconds wraps to 59 seconds while not borrowing from minutes).|
|`Countdown mode`| 1 | 0 |User setup inputs get ignored, except for the center button to stop the countdown (`sig_play_pause`). While the mode is active, it exclusively listens to `ce_1hz` pulse, executing standard clock math.|
|`Alarm ringing tone`| 0 | 1|When the timer reads 00.00.00, it triggers a red diode to glow and a buzzer to start making noise, continuing until the user presses the center button (`sig_play_pause`), returning the system to `Setup_mode`.|


<i>Note: `counting_down = 1`,`is_zero = 1`, does not happen, as the buzzer can't buzz and timer can't decrease at the same time</i>


Outcome of [`alarm_clock_tb.vhd`](alarm_clock/alarm_clock.srcs/sim_1/new/alarm_clock_tb.vhd):
...
## Custom module [`buzzer_module.vhd`]((alarm_clock/alarm_clock.srcs/sources_1/new/buzzer_module.vhd))
### Background
The nexys A7 50t board oscilator operates on a 100,000,000 frequency and we desire a 50% duty cycle square wave, which this module achieves (the actual used HW508 buzzer lacks an internal oscillator).

### I/O ports of the `buzzer_module.vhd` module:

Table of signals coming into custom module `buzzer_module.vhd`:
| Port name | Direction | Type | Description |
|:---------|:---------:|:----|:-----------|
...
Outcome of [`buzzer_module_tb.vhd`](alarm_clock/alarm_clock.srcs/sim_1/new/buzzer_module_tb.vhd): <br>
...
It is expected to connect the buzzer Signal pin to the rightmost corner of the JA pinout, the middle +3.3V pin to VCC pinout and the Ground on the GND pinout.
...
- Github: Documentation, repository home.
